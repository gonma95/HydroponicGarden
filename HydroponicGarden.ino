...
// WiFi access point
#define WIFI_AP_NAME        "vodafone4518"
// WiFi password
#define WIFI_PASSWORD       "GT9YAK3PDN2K8X"

// See https://thingsboard.io/docs/getting-started-guides/helloworld/ 
// to understand how to obtain an access token
#define TOKEN               "svyJof8VGEFAYo9SUdzQ"
...
int moisture_B; //humedad terreno en bruto
int moisture; //porcentaje Humedad terreno
...
void loop() {
  ...
  if (send_passed > send_delay) {
    ...
    water_lvl = analogRead(Water_lvl_PIN);
    if (isnan(water_lvl)) {
      Serial.println("Failed to read from Luminosity Sensor!");
    }
    else {
    ...
    moisture_B = analogRead(MOISTURE_PIN);
    if (isnan(MOISTURE_PIN)) {
      Serial.println("Failed to read from Luminosity Sensor!");
    }
    else {
      moisture = map(moisture, 0, 4095, 100 , 0);
      tb.sendTelemetryInt("Moisture", moisture);
      Serial.println(moisture_B);
    }
    send_passed = 0;
  }
